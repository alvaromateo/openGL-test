...
double fovy, dist, alpha_v, aspect;
double first_alpha_v;
...
void inicialitzacions()
{
	...
	dist = 2 * radi;
	first_alpha_v = asin(radi / dist);
	alpha_v = first_alpha_v;
...
void resize(int w, int h)
{
	...
	if (tipus == 0) {
		...
		if (ra_v < 1) {
			alpha_v = atan(tan(first_alpha_v) / ra_v);
			fovy = 2 * alpha_v * (180.0 / PI);
		}
		aspect = ra_v;
	}
	tipus_cam(tipus);
